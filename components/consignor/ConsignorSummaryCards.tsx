...
interface ConsignorSummaryCardsProps {
  total: number;
  active: number;
  inactive: number;
}

export function ConsignorSummaryCards({
  total,
  active,
  inactive,
}: ConsignorSummaryCardsProps) {
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      <div className="flex items-center gap-3 rounded-xl border border-border bg-white px-4 py-3 dark:bg-zinc-900">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">Tổng kho cung cấp</p>
        </div>
      </div>
      <div className="flex items-center gap-3 rounded-xl border border-border bg-white px-4 py-3 dark:bg-zinc-900">
