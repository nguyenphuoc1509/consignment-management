...
export function StoreNotFound({ id }: StoreNotFoundProps) {
  return (
    <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-border bg-white px-4 py-16 text-center dark:bg-zinc-900">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          Cửa hàng với mã &ldquo;{id}&rdquo; không tồn tại hoặc đã bị xóa.
        </p>
      </div>
      <Button variant="outline" size="sm" asChild>
        <Link href="/dashboard/stores">Quay lại danh sách</Link>
      </Button>
    </div>
  );
}
